...
export async function forgotPwHandler(
  ...
  try {
    ...
    <div class="container">
     <div class="content">
         <p class="heading">Please click on button below to verify your email.</p>
         <a class=" verify-button" href="${
           process.env.FRONTEND_URL
         }/forgot-password/${token}?email=${userEmail}&expiry=${expiryTime.getTime()}">Recover password</a>
     </div>
     <div class="footer">
...
export async function updatePasswordHandler(
  ...
  try {
    ...
    const user = await validatePassword({email:req.user.email,password:oldPw});

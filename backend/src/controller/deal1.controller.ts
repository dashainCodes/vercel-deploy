...
export async function createDealHandler(
  ...
  try {
    ...
    const img2 = await uploadSingleFile(companyLogo);
...
export async function getDealFromCategoryHandler(
  req: Request<ReadDealFromCategoryInput["params"]>,
  res: Response,
  next: NextFunction
) {
  try {
    const categoryId = req.params.categoryId;
    const deal = await findDeals({ category: { $eq: categoryId } });
